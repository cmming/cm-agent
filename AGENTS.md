...
- 新增或修改 Java 代码时，应主动判断哪些位置需要注释，不要求用户每次显式提出“补充注释”。
- 只要依据本节既有规则判断某处需要新增或更新注释、JavaDoc，Agent 必须在同一次代码修改中实际生成对应的中文注释或 JavaDoc；不得只在分析、计划、测试说明、最终回复或待办中提及而不落地。注释缺失即视为该处代码修改未完成。本条仅约束已由本节确定需要注释的代码，不扩大本节既有的注释适用范围。
- 注释的目标是帮助后续维护者理解设计意图、框架行为、约束、风险和容易产生疑问的地方，而不是机械描述代码字面含义。
- 优先解释“为什么这样做”，避免只解释“代码做了什么”。
- 注释密度应与语义复杂度匹配：
  - 配置项、常量、关键字段可使用一句简短说明；
  - 生命周期、资源管理、并发、事务、权限、安全、重试、降级、缓存、协议适配等行为应解释关键原因；
  - 框架隐式行为、版本差异、兼容性限制、容易踩坑的调用顺序应详细说明。
- 对容易让维护者产生疑问的代码，应主动补充“预判式注释”，提前回答可能的问题，例如：
  - 为什么这里需要或不需要显式初始化；
  - 为什么必须使用某个配置项或模式；
  - 资源由谁创建、持有和关闭；
  - 当前调用是同步、异步还是阻塞行为；
  - 某个默认值被省略后框架会采取什么行为；
  - 为什么不能使用看起来更直接的另一种实现；
  - 某段兼容代码、绕行逻辑或特殊判断为什么仍然需要保留。
- 使用第三方框架、SDK 或协议时，对代码中无法直接体现的重要隐式行为进行说明。尤其关注 AgentScope、Spring Boot、Spring Security、MCP、JDBC/Flyway、JWT、Testcontainers 等项目核心依赖。
- 对版本相关行为应在确有依据时明确标注，例如“AgentScope 2.0.0 下该 API 会……”；不得凭记忆编造版本特性。无法确认时应先通过当前依赖源码、官方文档、测试或实际行为验证。
- 对示例代码、集成示例和演示程序，应在类级 JavaDoc 中优先说明整体调用链和运行目标，使第一次接触相关框架的开发者无需先阅读全部实现即可理解执行流程。
- 示例类的类级 JavaDoc 可根据需要使用 `<ol>`、`<ul>`、`<p>` 等标准 JavaDoc HTML 结构描述整体步骤，但不要为了形式完整而堆砌冗长说明。
- 对重要方法使用 JavaDoc 说明职责、关键前置条件、资源生命周期、隐式副作用和特殊行为；简单私有辅助方法只在其语义无法从方法名和实现直接判断时补充说明。
- JavaDoc 中引用项目类型、方法或字段时，优先使用 `{@link ...}`；描述代码、配置键、方法名、协议名或字面量时优先使用 `{@code ...}`，保持 IDE 可导航性和文档可读性。
- 对公开 API、SPI、扩展点和可复用组件，应重点说明：
  - 输入约束；
  - 返回值语义；
  - 可观察副作用；
  - 失败方式；
  - 线程安全或生命周期要求；
  - 调用方必须遵守但类型系统无法表达的约束。
- 对安全相关代码，注释应明确说明安全边界和选择原因，例如权限绕过、开发模式 fallback、匿名访问、密钥读取、跨租户限制等。仅限示例或测试环境使用的弱化配置必须明确注明其适用范围和生产风险。
- 对 demo、example、测试夹具中为了简化演示而采用的非生产写法，应明确解释为什么可以在当前场景使用，以及生产环境应采用什么类别的替代方案。
- 不得因为“示例方便”而在注释、JavaDoc、README 或测试中写入真实 API Key、JWT secret、数据库密码或其他有效凭据；示例始终使用安全占位符或环境变量。
- 对异常处理代码，应优先解释异常被转换、包装、重试、降级或继续抛出的原因，以及调用方能够观察到的行为；不要只写“捕获异常”“打印错误”等重复实现的注释。
- 对权限、多租户和审计相关代码，应重点解释可信上下文来源、拒绝策略和为什么不能信任客户端传入值，避免维护者后续误删关键安全判断。
- 对复杂 SQL、Repository 查询和数据库迁移，应解释业务过滤条件、tenant 隔离要求、索引或兼容性原因；不要逐句翻译 SQL。
...
- 不使用下列低价值注释：
  ...
  - `// 返回结果`
  - `// 遍历列表`
  - 以及其他仅把下一行 Java 代码翻译成自然语言的注释。
- 更推荐类似以下类型的注释：
  - 说明某个调用内部已完成初始化，因此这里无需重复初始化；
  - 说明某个权限模式是为了演示跳过 human-in-the-loop，而生产环境不能照搬；
  - 说明保持原始顺序是因为下游依赖位置作为稳定 tie-breaker；
  - 说明特殊重试是为了应对已知上游瞬时故障，而不是隐藏所有异常；
  - 说明某个看似多余的判断是为了兼容特定版本或防止跨租户读取。
- 注释应与代码保持同步。修改行为、调用链、配置、异常策略或框架版本后，必须同时检查相关注释是否仍然准确；过期注释视为缺陷，不得保留。
- 删除或重构代码时，如果原注释描述的约束仍然存在，应迁移到新的正确位置；如果约束已不存在，应同时删除对应注释，禁止留下误导性历史说明。
- 不为了提高“注释覆盖率”而增加注释。评价标准是信息价值、准确性和维护价值，而不是注释数量。
- 默认遵循“简单代码少注释，复杂语义多解释；解释设计原因、隐式行为和维护陷阱，不重复语法”的原则。
