...
- 新增或修改 Java 代码时应主动判断是否需要注释，不要求用户每次显式提出“补充注释”；命中下方“触发清单”任意一条时，必须在同一次修改中直接落地对应中文注释或 JavaDoc，不能只在分析、计划、测试说明或回复中提及——未落地视为该处修改未完成，但不扩大到清单以外的代码。
- 本节规则对具体编码代理或模型一视同仁（含 GitHub Copilot、OpenAI Codex 等能读取本文件的代理），不得因某个代理默认输出习惯不同而省略、简化或改变要求；判断是否需要注释以“触发清单”为准，不凭代理自身风格取舍。
- 注释目标是帮助维护者理解设计意图、框架隐式行为、风险和易生疑问之处，优先解释“为什么这样做”而非复述代码字面含义；命名清晰、逻辑线性、无隐藏行为的简单赋值/校验/单一职责短方法可以不加注释，不为提升“注释覆盖率”而堆砌。
- 触发清单（命中任一条即必须补充/更新注释；密度随语义复杂度提升——配置项/常量一句话即可，框架隐式行为/版本差异/易踩坑调用顺序需详细说明）：
  ...
  - 生命周期、资源创建/持有/关闭、并发控制、事务边界；
  - 权限、安全边界、多租户过滤、审计判断：需说明可信上下文来源、拒绝策略、为什么不能信任客户端传入值，避免维护者后续误删关键安全判断；
  - 重试、降级、缓存策略、异常转换/包装/继续抛出：需说明原因及调用方可观察到的行为，不要只写“捕获异常”“打印错误”之类的复述；
  - 调用 AgentScope、Spring Boot、Spring Security、MCP、JDBC/Flyway、JWT、Testcontainers 等第三方框架时，存在代码本身无法体现的隐式行为、版本差异（须有实际依据，例如已验证的依赖源码、官方文档或测试，不得凭记忆编造）、兼容性限制；
  - 公开 API、SPI、扩展点或可复用组件的输入约束、返回值语义、可观察副作用、失败方式、线程安全或生命周期要求；
  - 复杂 SQL、Repository 查询、数据库迁移中的业务过滤条件、tenant 隔离、索引或跨数据库兼容性原因（不要逐句翻译 SQL）；
  - 示例代码、集成示例、demo 类，或其中仅适用于示例/测试的简化写法：需说明为何当前场景可用、生产环境应替换为什么类别方案；
  - 保留一段“看起来可以删除但实际仍需要”的兼容代码、绕行逻辑或特殊判断。
- 补充判断角度（用于确定注释具体该写什么内容，而非是否需要注释）：为什么这里需要/不需要显式初始化；资源由谁创建、持有、关闭；调用是同步、异步还是阻塞；默认值被省略后框架的实际行为；为什么不能用看起来更直接的实现。
- 示例代码、集成示例类应在类级 JavaDoc 说明整体调用链和运行目标，使初次接触相关框架的开发者无需先读完实现即可理解执行流程（可用 `<ol>`、`<ul>`、`<p>` 描述步骤，不为形式完整而堆砌）；重要方法用 JavaDoc 说明职责、关键前置条件、资源生命周期、隐式副作用；引用项目类型/方法/字段优先 `{@link ...}`，引用代码、配置键、方法名或字面量优先 `{@code ...}`。公开 API/SPI 方法示例（按需裁剪，不要求写满全部要素）：
  ```java
  /**
   * 使用 {@link ToolRegistry} 中已注册的工具执行一次同步调用。
   *
   * <p>调用方需自行保证 {@code request} 中的 tenantId 已通过认证上下文校验；
   * 本方法不会重复做租户校验。返回的 {@link ToolResult} 在失败时仍会包含
   * 已脱敏的错误原因和 {@code errorId}，不会返回 {@code null}。
   *
   * @param request 已完成参数校验的工具调用请求，不能为 {@code null}
   * @return 工具执行结果，失败时也返回非空对象而非抛出受检异常
   * @throws IllegalStateException 当工具未注册或已被禁用时抛出
   */
  ```
- 不使用低价值注释（例如 `// 设置用户名`、`// 创建对象`、`// 调用方法`、`// 返回结果`、`// 遍历列表` 等仅把下一行代码翻译成自然语言的注释）。正反示例对照：
  ```java
  // 反例：仅复述下一行代码，无信息增量
  for (Tool tool : tools) {
  ...
  // 正例：解释框架隐式行为，回答“为什么不需要先查重”
  // AgentScope 2.0.0 的 ToolRegistry#register 内部已完成幂等校验，
  // 重复注册同名工具会被静默忽略而非抛异常，因此这里无需先查重。
  for (Tool tool : tools) {
      registry.register(tool);
  }

  // 正例：解释安全边界和不可信输入来源，防止后续误删关键判断
  // tenantId 必须来自认证上下文，不能信任请求体中的 tenantId，
  // 否则会导致跨租户读取；此处显式忽略 request.tenantId() 字段。
  String tenantId = principal.getTenantId();
  ```
- 对安全相关代码（权限绕过、开发模式 fallback、匿名访问、密钥读取、跨租户限制等），注释必须说明安全边界和选择原因；仅限示例/测试环境使用的弱化配置必须注明适用范围和生产风险，且不得因“示例方便”写入真实凭据，始终使用安全占位符或环境变量。
- 对测试代码，仅在测试意图、边界条件、框架 setup 或回归原因不明显时添加注释；优先通过清晰的测试方法名和 Arrange/Act/Assert 结构表达常规测试逻辑。
- 注释需与代码保持同步：行为、调用链、配置、异常策略或框架版本变化后必须核对相关注释，过期注释视为缺陷不得保留；重构或删除代码时，原约束仍存在应迁移注释到新位置，约束已消失应同步删除，禁止留下误导性历史说明。评价注释质量的标准是信息价值、准确性和维护价值，而不是数量。
- 提交本次修改前自查（一次性对照“触发清单”，不重复展开细节，适用于任何生成或修改本仓库 Java 代码的代理）：命中项是否已落地为实际中文注释/JavaDoc 而非仅在回复中提及；新增内容是否解释“为什么”而非复述代码；是否出现低价值注释模式；本次修改是否影响了既有注释描述的行为，若已变化是否同步更新或删除。
