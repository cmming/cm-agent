...
- 当前阶段：Task 1～Task 11 已实现并完成针对性回归。
...
- 新增 v2 技能管理页面和独立脚本，沿用控制台“列表—详情—操作”结构及统一认证客户端；全部 v2 导航增加技能入口。
- 页面支持 ZIP 导入、详情、启停与固定版本资源按需预览。正文不进入技能列表或系统提示，仅在用户选择资源后调用受控接口读取。
- 所有 v2 独立页面预加载技能脚本，技能页在应用脚本前加载它；因此多页导航替换页面主体后，仍能复用已经创建的技能页面组件。

### Task 10 实际实现

- Agent 详情新增独立技能绑定区域：以会话代际和加载修订号防止旧 Agent 的迟到响应写回当前详情；只有同时拥有 `agent:write` 与 `skill:read` 时显示已启用技能候选、绑定和解绑操作，服务端仍是最终授权边界。
- Run 详情在既有主体、输出和工具调用之后异步查询读取记录。每个条目仅展示固定版本号、路径、状态、交付字节、耗时、时间和错误关联编号，不读取或渲染技能正文。
- 公共前端脚本增加技能读取状态格式化函数及 Node 断言；读取列表的加载或失败只替换本地区域，快速切换 Run 或退出登录时由修订号和会话代际丢弃迟到响应。

### Task 11 实际实现

- v2 全部页面和登录页更新共享脚本的缓存版本；控制台资源测试覆盖九个页面、技能导航与当前脚本版本，防止浏览器继续使用旧版多页控制逻辑。
- `README.md`、配置、运维、发布说明和 Adapter README 已同步为可交付说明，明确开关默认关闭、权限组合、ZIP 文本边界、固定版本、撤销恢复及排障接口。
- 最终本地回归覆盖 Core、AgentScope Adapter、控制台资源、管理 API、快照与读取治理；先前 T4～T6 的 Rocky PostgreSQL/MySQL 验证仍适用于本次未变更的 V12/JDBC 链路。
...
- 应用户后续要求，ZIP 内文本资源扩展名改为 `cm-agent.skills.allowed-resource-types` 配置项；默认配置开放 `.html`、`.js`、`.cjs` 和 `.sh`，解析器与能力接口读取同一份配置，容量限制仍保持只允许向下收紧。
...
正式说明已在 T11 同步更新。浏览器人工全流程和以真实 Provider Stub 驱动的完整模型选择行为未在本次本地回归中重跑；现有 AgentScope 契约测试、控制台资源测试和管理/治理测试覆盖对应边界，但不将其表述为真实模型演示。
