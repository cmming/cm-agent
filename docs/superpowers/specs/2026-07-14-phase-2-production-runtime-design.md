...
阶段 2 的实现范围与验收项保持不变。本次复核发现全量测试被后续全局错误诊断改动阻断：`McpServerConfiguration` 新增 `ErrorDiagnosticLogger` 依赖后，其隔离配置测试未同步提供该依赖，导致 MCP 配置用例在业务断言前启动失败。

该问题只修复测试装配，不改变阶段 2 的运行、持久化、安全、profile 或迁移语义。按用户指示，不处理当前工作区中与阶段 2 无关的配置文件问题；最终验收以干净提交加本次受控改动的 Rocky VM 容器测试为准，并记录在进度账本。
