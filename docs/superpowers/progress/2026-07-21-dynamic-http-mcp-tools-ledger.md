...
| Task 4：输入校验与参数映射 | 已完成 | 六轮复审最终补齐 `dependentSchemas` 同实例环及跨引用链缓存绕过；既有 RED 覆盖 discriminator oneOf、Schema-bearing 子树引用及数组索引安全上限 | Validator 24、Mapper 13、Management 5、ToolController 7，共 49 项通过；networknt 依赖树确认 2.0.0 | `dependentSchemas` 传播同实例引用链；移除忽略链上下文的全局 path 缓存，仅保留递归栈与直接引用链；合法 properties/items 递归不受影响 | `29b01fa`、`e4587a9`、`537274f`、`37caa27`、`0dfde42`、`ac35d45`、本提交 |
