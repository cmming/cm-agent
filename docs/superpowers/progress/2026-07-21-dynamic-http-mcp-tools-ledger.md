...
| Task 4：输入校验与参数映射 | 已完成 | 七轮复审最终补齐重复引用 DAG 性能与宽 Schema 复杂度上限；既有 RED 覆盖 discriminator oneOf、Schema-bearing 子树引用、数组索引安全上限及引用链绕过 | Validator 26、Mapper 13、Management 5、ToolController 7，共 51 项通过；networknt 依赖树确认 2.0.0 | 以 schema path 与不可变直接引用链作为完成态缓存键；单次配置校验最多遍历 10,000 个调用局部状态，超限固定拒绝；历史环检测和合法结构递归均保持 | `29b01fa`、`e4587a9`、`537274f`、`37caa27`、`0dfde42`、`ac35d45`、`87ad973`、本提交 |
