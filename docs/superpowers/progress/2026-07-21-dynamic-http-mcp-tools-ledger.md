...
| Task 5：安全 HTTP 执行器 | 已完成 | Secret/属性、URL 策略、执行器、profile 均先观察到缺类、缺行为或错误语义失败；复审 RED 继续覆盖跨源重定向、统一 deadline、IPv6 特殊前缀、歧义响应头、结构化脱敏及阻塞/累计 Secret Provider | Secret、URL、执行器、profile 最终指定 143 项通过；跳过测试的 reactor 打包通过 | 同源重定向、严格 profile、统一 deadline、可关闭传输、保守 IPv6、identity 编码、递归 JSON/text 脱敏和序列化后二次限长已补齐；完整 Secret 循环纳入同一 deadline，超时后 DNS/网络零调用。自定义 Provider 必须响应中断并配置自身 I/O timeout；忽略中断时调用方可及时返回，但后台虚拟线程可能等到 Provider 返回。JDK HttpClient 仍保留 DNS TOCTOU 风险并要求基础设施 egress | `e7e444c`、`041929d`、本提交 |
