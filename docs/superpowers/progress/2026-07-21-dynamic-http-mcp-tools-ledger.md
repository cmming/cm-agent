...
| Task 5：安全 HTTP 执行器 | 已完成 | Secret/属性、URL 策略、执行器、profile 均先观察到缺类、缺行为或错误语义失败；本地 HttpServer 覆盖实际 GET/POST/重定向/大小/超时 | Secret、URL、执行器指定 66 项通过；加入 profile 的指定 104 项回归通过；跳过测试的 reactor 打包通过 | 补齐 Boot 后置自动配置覆盖、属性不可突破的硬边界、构造器注入、IDN canonical URI、Windows 稳定连接异常测试及 Authorization/Cookie/完整 URL 脱敏；JDK HttpClient 无法绑定已校验 IP，保留 DNS TOCTOU 风险并要求基础设施 egress | 本提交 |
