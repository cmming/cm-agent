...
| Task 4：输入校验与参数映射 | 已完成 | 目标类缺失导致新测试编译失败；Management 未调用 Validator 导致非法 Schema 未拒绝；四轮复审 RED 继续覆盖 discriminator oneOf 局部投影、Schema-bearing 子树引用及数组索引安全上限 | Validator 21、Mapper 13、Management 5、ToolController 7，共 46 项通过；networknt 依赖树确认 2.0.0 | 局部 oneOf 降为候选子 Schema 的 anyOf，完整输入保留根 oneOf；递归检查终点 Schema 子树本地引用并区分合法结构递归；三处统一使用 `HttpToolArrayIndex`，最大下标 10,000 | `29b01fa`、`e4587a9`、`537274f`、`37caa27`、本提交 |
