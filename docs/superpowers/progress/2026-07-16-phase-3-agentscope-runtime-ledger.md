...
| Task 4：工具治理网关 | 已完成 | RED：JDK 21.0.11、Maven 3.9.4 下目标测试编译因缺少 `GovernedToolInvocationService` 失败；GREEN：目标 9 个测试通过；15 个非容器 Server 测试类回归共 128 个测试通过；`git diff --check` 通过 | 首次 GREEN 的完成审计失败用例因 Mockito 严格桩未正确表达同一 `append` 的连续调用，触发 `PotentialStubbingProblem`；本机 Server 全量测试包含 3 个 Testcontainers/JDBC Web 测试，按仓库规则不能在本机 Docker 执行 | 使用 `doNothing().doThrow(...)` 精确模拟开始审计成功、完成审计失败，未改变生产语义；目标和非容器回归均通过，且本任务未涉及 JDBC、Flyway 或容器变更 |
